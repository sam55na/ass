...
const gamesCache = new NodeCache({ stdTTL: 60, checkperiod: 120 }); // كاش للألعاب
...
// ============= إدارة الألعاب الديناميكية =============
const GAMES_COLLECTION = 'games';

async function getGames(includeDisabled = false) {
  // محاولة جلب من الكاش أولاً
  const cacheKey = includeDisabled ? 'all_games' : 'enabled_games';
  const cached = gamesCache.get(cacheKey);
  if (cached) return cached;
  
  try {
    let query = db.collection(GAMES_COLLECTION);
    if (!includeDisabled) {
...
async function saveGame(gameData, gameId = null) {
  const gamesRef = db.collection(GAMES_COLLECTION);
  let docRef;
  
  if (gameId) {
    docRef = gamesRef.doc(gameId);
    await docRef.update({
      ...gameData,
      updatedAt: new Date()
    });
    ...
    // حساب ترتيب جديد
    const existingGames = await getGames(true);
    const maxOrder = existingGames.reduce((max, g) => Math.max(max, g.order || 0), 0);
    docRef = gamesRef.doc();
    await docRef.set({
      ...gameData,
      order: maxOrder + 1,
...
async function toggleGameStatus(gameId, enabled) {
  try {
...
async function updateGameOrder(gameId, newOrder) {
  try {
    await db.collection(GAMES_COLLECTION).doc(gameId).update({
      order: newOrder,
      updatedAt: new Date()
...
app.post('/api/admin/games', requireAdmin, async (req, res) => {
  try {
    const { name, gameUrl, iconUrl, enabled = true } = req.body;
    
    if (!name || !gameUrl) {
      return res.status(400).json({ error: 'اسم اللعبة ورابط اللعبة مطلوبان' });
    }
    ...
    res.json({ success: true, message: 'تم إضافة اللعبة بنجاح', gameId });
  } catch (error) {
    console.error('Error adding game:', error);
    res.status(500).json({ error: 'خطأ في إضافة اللعبة' });
  }
});
...
app.put('/api/admin/games/:gameId', requireAdmin, async (req, res) => {
  try {
    ...
    const { name, gameUrl, iconUrl, enabled } = req.body;
    ...
    if (gameUrl !== undefined) updates.gameUrl = gameUrl.trim();
    if (iconUrl !== undefined) updates.iconUrl = iconUrl?.trim() || '';
    ...
    await saveGame(updates, gameId);
    res.json({ success: true, message: 'تم تحديث اللعبة بنجاح' });
  } catch (error) {
    console.error('Error updating game:', error);
    res.status(500).json({ error: 'خطأ في تحديث اللعبة' });
  }
});
...
app.delete('/api/admin/games/:gameId', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('Error deleting game:', error);
    res.status(500).json({ error: 'خطأ في حذف اللعبة' });
  }
});

app.patch('/api/admin/games/:gameId/toggle', requireAdmin, async (req, res) => {
  try {
    const { gameId } = req.params;
    const { enabled } = req.body;
    
    if (typeof enabled !== 'boolean') {
      return res.status(400).json({ error: 'حالة التفعيل غير صالحة' });
    }
    
    const success = await toggleGameStatus(gameId, enabled);
    if (success) {
      res.json({ success: true, message: enabled ? 'تم تفعيل اللعبة' : 'تم تعطيل اللعبة' });
    } else {
      res.status(404).json({ error: 'اللعبة غير موجودة' });
    }
  } catch (error) {
    console.error('Error toggling game:', error);
    res.status(500).json({ error: 'خطأ في تغيير حالة اللعبة' });
  }
});

app.patch('/api/admin/games/:gameId/order', requireAdmin, async (req, res) => {
  try {
    const { gameId } = req.params;
    const { order } = req.body;
    
    if (typeof order !== 'number' || order < 0) {
      return res.status(400).json({ error: 'الترتيب غير صالح' });
    }
    
    const success = await updateGameOrder(gameId, order);
    if (success) {
      res.json({ success: true, message: 'تم تحديث ترتيب اللعبة' });
    } else {
      res.status(404).json({ error: 'اللعبة غير موجودة' });
    }
  } catch (error) {
    console.error('Error updating game order:', error);
    res.status(500).json({ error: 'خطأ في تحديث ترتيب اللعبة' });
  }
});

// ============= API جلب الألعاب للمستخدمين =============
...
app.get('/api/games/:gameId', async (req, res) => {
  try {
    ...
    if (!game.enabled) {
      return res.status(403).json({ error: 'اللعبة غير متاحة حالياً' });
    }
...
const server = app.listen(PORT, () => {
  ...
  console.log(`🎮 Games management system ready`);
